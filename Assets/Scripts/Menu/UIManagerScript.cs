...
public class UIManagerScript : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        if (PlayerPrefs.GetInt("Camera") == 0) { cameraChoose.isOn = false; cameraTwo.SetActive(true); }
        
        else if (PlayerPrefs.GetInt("Camera") == 1) { cameraChoose.isOn = true; cameraOne.SetActive(true); }
    ...
    public void Start()
    {
        ...
        kills.text = "Your record: " + PlayerPrefs.GetInt("Record").ToString() + " km\n You play in game " + time.ToString() + " m";

        for (int i = 2; i < 10; i++) //перевірка на досягнення рекорду та подальше його відкриття 
        {
    ...
    public void Changed_controller()
    {
        ...
        {
            if (type != 0) PlayerPrefs.SetInt("Camera", 1);
        }
        else 
        {
            if (type != 0) PlayerPrefs.SetInt("Camera", 0);
        }
        PlayerPrefs.Save();
        cameraOne.SetActive(false);
        cameraTwo.SetActive(false);
        if (type == 1) SceneManager.LoadScene("Scene0");
        type++;
    }
    ...
    private void block_achi()
    {
        ...
        int killed = PlayerPrefs.GetInt("Record")/5;
